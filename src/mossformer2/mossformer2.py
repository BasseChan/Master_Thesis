...
class MossFormer2_adapted(nn.Module):
    """MossFormer2 model wrapper for outside calling"""

    def __init__(self, checkpoint_path='mossformer2/checkpoints'):
        super(MossFormer2_adapted, self).__init__()
        ...
        self.model = MossFormer(
            ...
            skip_around_intra=True,
            use_global_pos_enc=True,
            max_length=20000).to(self.device)
        load_state_dict(self.model, checkpoint_path)
    ...
    def forward(self, x):
        ...
        self.model.eval()
        with torch.no_grad():
            output_audios = decode_one_audio(self.model, self.device, x)
            output = []
            for spk in range(2):
